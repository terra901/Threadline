...
| 导入 / 导出 | 支持全库备份、单个会话图谱导出、平台原始导出导入。 |
...
`memories` 表的重要字段包括：`id`、`role`、`content`、`provider`、`sessionId`、`timestamp`、`turnIndex`、`roundIndex`、`branchIndex`、`branchId`、`pathId`、`parentMessageId`、`chunkIndex`、`parentId`、`embedding`、`hasEmbedding`、`metadata`。
...
```text
query -> 本地 embedding -> 带时间衰减的向量检索
      -> BM25 关键字检索
      -> RRF 倒数排名融合
      -> top-k Recall 结果
```

长消息会在 embedding 前按 500 字符分块，分块重叠 75 字符。Memory Graph 展示时会把 chunk 合并回逻辑消息。

## 导入与导出

- Threadline 全库备份使用 `metadata.app = "Threadline"` 和 `payload` 数组。
- 旧版 `PersonalAIMemoryLayer` 备份仍可兼容导入。
- Memory Graph 可以导出单个 session，格式为 `ThreadlineSessionGraph`。
- 平台导入支持 ChatGPT、Claude、Gemini Takeout、Grok。Perplexity 需要访问具体 thread 后自动捕获。
