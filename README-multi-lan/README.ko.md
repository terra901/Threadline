...
Recall 결과는 AI 입력창 위에 표시되며, 선택한 원문만 composer에 주입하거나 Memory Graph에서 원본 메시지를 열 수 있습니다.
...
Memory Graph는 저장된 session과 pending session, provider filter, session actions, message count, vector 상태, zoom control, branch path를 보여줍니다.
